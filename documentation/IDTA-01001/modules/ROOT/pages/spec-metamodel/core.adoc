...
....
plantuml::partial$diagrams/24-asset-administration-shell.puml[]
....
...
For more detailed information on the _derivedFrom_ concept, see Clause xref:general.adoc#types-and-instances[Types and Instances].
...
....
plantuml::partial$diagrams/25-asset-information.puml[]
....
...
h|Explanation: 3+a|
In _AssetInformation,_ identifying metadata of the asset that is represented by an Asset Administration Shell is defined.
...
a|Denotes whether the _Asset_ is of kind "Type", Instance", "Role" or none of these types is applicable |xref:AssetKind[AssetKind]|1
...
====
Note: in case _AssetInformation/assetKind_ is "Instance" then the AssetInformation/assetType denotes which "Type" the asset is of.
But it is also possible to have an _AssetInformation/assetType_ of an asset of kind "Type".
====
...
....
plantuml::partial$diagrams/26-submodel.puml[]
....
...
....
plantuml::partial$diagrams/27-submodel-element.puml[]
....
...
Otherwise, there is only the property definition referenced by _semanticId_ available for the property; the attributes must be looked up online in a different way and are not available offline.
