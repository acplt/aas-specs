...
image:ds/media/image1.png[image,width=802,height=1145]
...
Subclause <<link:#terms-definitions-and-abbreviations,2>> provides terms and definitions as well as abbreviations, both for abbreviations used in the document and for abbreviations that may be used for elements of the metamodel defined in this document.

Subclause <<link:#introduction,3>> gives a short introduction of Asset Administration Shell types and how this document is related to them.

Subclause <<link:#general,4>> explains the purpose of the data specification template specified in this document by giving examples of existing data dictionaries.

Subclause <<link:#predefined-data-specification-templates,5>> shows how the data specification template is related to Part 1 and its elements.

Subclause <<link:#predefined-template-for-iec61360-properties-value-lists-and-values-normative,6>> is the main normative part of the document. It specifies the data specification templates supporting IEC 61360.

Subclause <<link:#mapping-iec-61360-data-types-to-xsd-data-types,7>> explains how data types of IEC 61360 are mapped to data types of values as introduced in Part 1.

Subclause <<link:#category-of-concept-descriptions,8>> introduces categories for concept descriptions and how they are used in combination with the data specification template IEC61360. The constraints as defined in Clause 0 also mainly refer to the rules on how these categories should be applied.


====
Note: since categories are deprecated since V3.0, Subclause <<link:#category-of-concept-descriptions,8>> can also be skipped.
====


Subclause <<link:#primitive-and-simple-data-types-normative,10>> specifies the data types used in the data specification.
...
Finally, Subclause <<link:#summary-and-outlook,12>> summarizes the content and gives an outlook on future work.

The Annex contains additional background information on the Asset Administration Shell (<<link:#background-information,Annex A>>). It also provides information about UML (<<link:#legend-for-uml-modelling,Annex D>>) and the tables used to specify UML classes as used in this specification (<<link:#templates-for-uml-tables,Annex C>>). <<link:#backus-naur-form,Annex B>> introduces the Backus-Naur-Form used in the document series.

Metamodel changes compared to previous versions are described in <<link:#_Toc129945523,Annex E>>.
...
<<link:#_Ref55938616,Figure 1>> shows the different ways of exchanging information via Asset Administration Shells. This part of the "Asset Administration Shell in Detail" series is the basis for all of these types of information exchange.

[#_Ref55938616]
.Types of Information Exchange via Asset Administration Shells
image:ds/media/image2.jpeg[image,width=535,height=337]
...
image:ds/media/image4.png[image,width=560,height=366]




image:ds/media/image5.png[image,width=396,height=112]
...
image:ds/media/image6.png[image,width=406,height=296]



[#_Ref129950722]
.Example for Property with Level Type from IEC CDD
image:ds/media/image7.png[image,width=642,height=632]
...
The legend for understanding the UML diagrams and the table specification of the classes is explained in <<link:#templates-for-uml-tables,Annex C>> and <<link:#legend-for-uml-modelling,Annex D>>.
...
=== Data Specification IEC61360 Attributesimage:ds/media/image9.emf[image,width=473,height=440]

[#_Ref122380484]####Figure _8_ Metamodel of Data Specification IEC61360
...
=== image:ds/media/image10.emf[image,width=178,height=333]
...
=== image:ds/media/image11.emf[image,width=103,height=108]
...
image:ds/media/image12.emf[image,width=169,height=182]
...
Although the IEC 61360 attributes listed in this template are defined for properties and values only, it is also possible to use the template for other definitions as long as no specific data specifications for concept descriptions for these elements are available. This is shown in Table 1, <<link:#_Ref129944411,Table 2>> and <<link:#_Toc24314355,Table 4>>.
...
See <<link:#_Ref129879750,Figure 7>> Overview Relationship Metamodel Part 1 a & Data Specifications IEC 61360 on how data specification templates are related to concept descriptions. <<link:#_Ref91684559,Figure 13>> lists all categories used for concept descriptionsfootnote:[Note: although the possible categories are listed as enumeration, no enumeration has been defined for Referable/category.].

[#_Ref91684559]
.Categories of Concept Descriptions (non-normative)
image:ds/media/image13.png[image,width=255,height=343]
...
<<link:#_Toc122380103,Table 6>> lists the Primitives used in the metamodel. Primitive data types start with a capital letter.
...
image:ds/media/image16.png[image,width=565,height=730]




<<link:#_Ref129879750,Figure 7>> shows a property "CoolingType". Its semanticId references a concept description that defines a value list (_DataSpecificationIec612360/valueList_) with two values BAB657 and BAB611.

[#_Ref129950911]
.Example Property with Enumeration in AASX Package Explorer
image:ds/media/image17.png[image,width=545,height=770]



<<link:#_Ref129879857,Figure 18>> shows the concept description for the value BAB657 that was used in the enumeration in <<link:#_Ref129950911,Figure 17>>. Most attributes are not relevant (see Clause 8). However, it is mandatory to set the attribute _DataSpecificationIec61360/value_, the _preferredName_ (open circuit, external cooling), and the data type (for enumeration, the data type is typically just STRING).

[#_Ref129879857]
.Example Value Concept Description in AASX Package Explorer
image:ds/media/image18.png[Ein Bild, das Text enthält. Automatisch generierte Beschreibung,width=639,height=368]



=== Referencing (Part 1)

Besides the abstract class _HasSemantics,_ the referencing concept explained in Part 1 is also relevant (type __Reference__<<link:#_Ref129879893,Figure 19>>). In the case of the data specification template IEC61360, the only relevant key types are "GlobalReference" and "ConceptDescription". In case the concept description is a shadow copy of an existing data dictionary and uses the same ID, it is recommended to use the Global Reference for the _DataSpecificationIec61360/unitId_ or _ValueReferencePair/valueId_. Otherwise, a model reference with _Key/type_ equal to _ConceptDescription_ is used.
...
image:ds/media/image19.emf[image,width=424,height=502]
...
[#_Ref129879956]
.Multiplicity
image:ds/media/image22.emf[image,width=200,height=80]
...
image:ds/media/image23.emf[image,width=233,height=80]

A multiplicity element represents a collection of values. The default is a set, i.e. it is not ordered and the elements within the collection are unique and contain no duplicates. <<link:#_Ref125977674,Figure 23>> shows an ordered collection: the instances of _Class2_ related to an instance of _Class1_. The stereotype \<<ordered>> is used to denote that the relationship is ordered.

[#_Ref125977687]
.Association
image:ds/media/image23.emf[image,width=233,height=80]

.shows that the member ends of an association can be named as well, i.e. an instance of _Class1_ can be in relationship "relation" to an instance of _Class2_. Vice versa, the instance of _Class2_ is in relationship "reverseRelation" to the instance of _Class1_.

[#_Ref125977695]
.Composition (Composite Aggregation)
image:ds/media/image24.emf[image,width=200,height=80]

.shows a composition, also called a composite aggregation. A composition is a binary association, grouping a set of instances. The individuals in the set are typed as specified by _Class2_. The multiplicity of instances of _Class2_ to _Class1_ is always 1 (i.e. upper-bound and lower-bound have value "1"). One instance of _Class2_ belongs to exactly one instance of _Class1_. There is no instance of _Class2_ without a relationship to an instance of _Class1_. <<link:#_Toc129706750,Figure 26>> shows the composition using an association relationship with a filled diamond as composition adornment.

<<link:#_Toc129706750,Figure 26>> shows an aggregation. An aggregation is a binary association. In contrast to a composition, an instance of _Class2_ can be shared by several instances of _Class1_. <<link:#_Toc129706750,Figure 26>> shows the shared aggregation using an association relationship with a hallow diamond as aggregation adornment.

[#_Toc129706750]
Figure _26_ Aggregation
image:ds/media/image25.emf[image,width=200,height=80]
...
image:ds/media/image26.emf[image,width=163,height=80]

.illustrates that the attribute notation can be used for an association end owned by a class. In this example, the attribute name is "attr" and the elements of this attribute are typed with _Class2._ The multiplicity, here "0..*", is added in square brackets. If the aggregation is ordered, it is added in curly brackets like in this example.

[#_Ref125977889]
.Default Value
image:ds/media/image27.emf[image,width=91,height=80]

.shows a class with three attributes with primitive types and default values. When a property with a default value is instantiated in the absence of a specific setting for the property, the default value is evaluated to provide the initial values of the property.

[#_Ref125977898]
.Dependency
image:ds/media/image28.emf[image,width=206,height=80]

.shows that there is a dependency relationship between _Class1_ and _Class2_. In this case, the dependency means that _Class1_ depends on _Class2_ because the type of attribute _attr_ depends on the specification of class _Class2._ A dependency is depicted as dashed arrow between two model elements.

[#_Ref125977906]
.Abstract Class
image:ds/media/image29.emf[image,width=86,height=80]

.shows an abstract class. It uses the stereotype \<<abstract>>. There are no instances of abstract classes. They are typically used for specific member elements that are inherited by non-abstract classes.

[#_Ref125977915]
.Package
image:ds/media/image30.emf[image,width=106,height=80]

.shows a package named "Package2". A package is a namespace for its members. In this example, the member belonging to _Package2_ is class _Class2_.

[#_Ref125977922]
.Imported Package
image:ds/media/image31.emf[image,width=256,height=86]

.shows that all elements in _Package2_ are imported into the namespace defined by _Package1_. This is a special dependency relationship between the two packages with stereotype \<<import>>.

[#_Ref125977933]####Figure _33_ Enumerationfootnote:[In Enterprise Architect, the single enumeration values also have a stereotype \<<enum>> each.]
image:ds/media/image32.emf[image,width=119,height=93]

Figure _33_ shows an enumeration with the name "Enumeration1". An enumeration is a data type with its values enumerated as literals. It contains two literal values, "a" and "b". It is a class with stereotype \<<enumeration>>. The literals owned by the enumeration are ordered.
image:ds/media/image33.emf[image,width=86,height=73].shows the definition of the data type with the name "DataType1". A data type has instances that are identified only by their value. It is a class with stereotype \<<dataType>>.
...
[#_Ref125977948]
.Primitive Data Type
image:ds/media/image34.emf[image,width=86,height=73]

.shows a primitive data type with the name "int". Primitive data types are predefined data types, without any substructure. The primitive data types are defined outside UML.

[#_Ref125977955]
.Note
image:ds/media/image35.emf[image,width=200,height=80]

.shows how a note can be attached to an element, in this example to class "Class1".

[#_Ref125977964]
.Constraint
image:ds/media/image36.emf[image,width=213,height=80]

.shows how a constraint is attached to an element, in this example to class "Class1".
...
image:ds/media/image37.emf[image,width=410,height=205]
...
[#_Ref125545522]####Figure _39_ Graphical Representation of Shared Aggregation
image:ds/media/image38.emf[image,width=368,height=220]

A specialty in Figure _39_ is that the aggregated instances are referables in the sense of the Asset Administration Shell metamodel (i.e. they inherit from the predefined abstract class "Referable"). This is why Variant B is identical to Variant A. This would not be the case for non-referable elements in the metamodel. The structure of a reference to a model element of the Asset Administration Shell is explicitly defined. A model reference consists of an ordered list of keys. The last key in the key chain shall reference an instance of type _Class2_ (i.e. Reference/type equal to "Class2").
<<link:#_Ref125545524,Figure 40>> show different graphical representations of generalization. Variant A is the classical graphical representation as defined in link:#bib12[[12\]]. Variant B is a short form, if _Class1_ is not on the same diagram. The name of the class that _Class3_ is inheriting from is depicted in the upper right corner.
...
image:ds/media/image39.emf[image,width=386,height=206]




<<link:#_Ref129880067,Figure 41>> depicts different graphical notations for enumerations in combination with inheritance. In Variant A, "Enumeration1" additionally contains the literals as defined by "Enumeration2".

[#_Ref129880067]
.Graphical Representation for Enumeration with Inheritance
image:ds/media/image40.emf[image,width=339,height=160]
...
<<link:#_Ref129880077,Figure 42>> shows an experimental class, marked by the stereotype "Experimental".

[#_Ref129880077]
.Graphical Representation for Experimental Classes
image:ds/media/image41.png[image,width=119,height=96]



<<link:#_Ref129880119,Figure 43>> depicts a deprecated class, which is marked by the stereotype "Deprecated".

<<link:#_Ref129880129,Figure 44>> shows a class representing a template. It is marked by the stereotype "Template".

[#_Ref129880119]
.Graphical Representation for Deprecated Elements
image:ds/media/image42.png[image,width=123,height=99]



[#_Ref129880129]
.Graphical Representation of a Template Class
image:ds/media/image43.png[image,width=120,height=97]
