...
Table for attributes look like this.
Please remember to use the table descriptions.
...
The attributes `type` and "local" (typically `ConceptDescription` and "(local)" or `GlobalReference` and (no-local)") need to be set accordingly; see link:#bib6[[6\]].
